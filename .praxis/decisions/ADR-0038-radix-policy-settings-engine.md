...
- **`praxis/procedures/constraint-eval.px`** — the existing **constraint model**: `constraint:<name>` keys hold a definition (`given`, `check`, `severity: error|warning|log`), stored/updated via `pluresdb_write {key: "constraint:<name>", value: <definition>}`. Evaluation flow: `evaluate_constraint` (single) → `evaluate_all_constraints` (loop over `db_get_prefix {prefix: "constraint:"}`) → `enforce_violation` (queue-driven off a `constraint_violation` fact, dispatches `{action: block|warn|log, reason}` based on severity). This is a real, already-shipping **policy engine primitive**: named rule, prefix-scoped storage, severity-driven enforcement, queue/dataflow wiring (`-> "queue_name"` / `from "queue_name"`).
