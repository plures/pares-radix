...
describe('ui-practices drift guard', () => {
  ...
  it('every practice carries a non-empty author-facing rationale', () => {
    // Each rationale must be present in BOTH the .px and the TS mirror and be a
    // real sentence (the guidance-on-override layer surfaces it on deviation).
    for (let i = 0; i < UI_PRACTICES.length; i++) {
      const px = parsed[i];
      expect(px.rationale && px.rationale.trim().length, `${px.name} .px rationale`).toBeGreaterThan(0);
      expect(UI_PRACTICES[i].rationale.trim().length, `${UI_PRACTICES[i].name} TS rationale`).toBeGreaterThan(0);
    }
  });
