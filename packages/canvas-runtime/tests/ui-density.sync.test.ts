/**
 * Drift guard (C-DRIFT-001): praxis/ui/ui-density.px  ↔  UI_DENSITY_PRACTICES.
 *
 * The .px file is the human-readable source of truth; UI_DENSITY_PRACTICES
...
import { UI_DENSITY_PRACTICES, type UiPractice } from '../src/ui-practices.js';

const __dirname = dirname(fileURLToPath(import.meta.url));
// tests/ -> packages/canvas-runtime -> packages -> repo root
const PX_PATH = resolve(__dirname, '../../../praxis/ui/ui-density.px');
...
describe('ui-density practices drift guard', () => {
  ...
  it('same number of practices in .px and UI_DENSITY_PRACTICES', () => {
    expect(parsed.length).toBe(UI_DENSITY_PRACTICES.length);
  });

  it('each practice matches name/kind/appliesTo/set/source/when in order', () => {
    for (let i = 0; i < UI_DENSITY_PRACTICES.length; i++) {
      const ts = normalizeTs(UI_DENSITY_PRACTICES[i]);
      const px = parsed[i];
  ...
  it('every practice carries a non-empty author-facing rationale', () => {
    for (let i = 0; i < UI_DENSITY_PRACTICES.length; i++) {
      ...
      expect(UI_DENSITY_PRACTICES[i].rationale.trim().length, `${UI_DENSITY_PRACTICES[i].name} TS rationale`).toBeGreaterThan(0);
    }
  });

  it('every practice sets a real spacing prop (padding | gap)', () => {
    // Honesty invariant at the practice level: density only ever scales the two
    // real Box spacing props (both also in RESPONSIVE_ATTRS).
    const known = new Set(['padding', 'gap']);
    for (const p of UI_DENSITY_PRACTICES) {
      expect(known.has(p.set), `${p.name} sets non-spacing attr ${p.set}`).toBe(true);
    }
  });
});
